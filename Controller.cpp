...
void MyInterface::input_file_changed(Fl_Widget *wid, void *pVoid) {
    ...
    char buf[1000];
    memset(buf, 0, 1000);
    fl_utf8to_mb(p->input_file_edt->value(), static_cast<unsigned int>(strlen(p->input_file_edt->value())), buf, 999);
...
char *mb(const char *p) {
    memset(buf, 0, 1000);
    fl_utf8to_mb(p, static_cast<unsigned int>(strlen(p)), buf, 999);
    return buf;
}
char *mb(const string &s) {
    memset(buf, 0, 1000);
    fl_utf8to_mb(s.c_str(), static_cast<unsigned int>(strlen(s.c_str())), buf, 999);
    return buf;
}
    ...
    try {
        ...
        while (input_f) {
            ...
            if (ls[1] == '3') {
                ...
                unsigned int tst = v2 + v - adr_begin;
                if (tst > len)
                    len = tst;
            }
        }
        ...
        tmp_v.resize(len);
        (unsigned char) stoul(inp_ch->value(), nullptr, 16);
//        generate(tmp_v.begin(), tmp_v.end(),[] {return val;} );
        ...
        while (input_f) {
            ...
            if (ls[1] == '3') {
                ...
                v2 -= (4 + 1);  // remove address and cs
                for (auto i = 0; i < v2; ++i) {
                    string s3 = ls.substr(static_cast<unsigned long long int>(12 + i * 2), 2);
                    auto val = stoul(s3, nullptr, 16);
                    tmp_v[cur_addr - adr_begin + i] = static_cast<unsigned char>(val);
                }
            }
        }
