...
//    col_width_all(25);
    row_header_width(80);
    m = new Fl_Menu_Button(0,0,700,400, NULL);
    Fl_Menu_Button &mb = *m;
...
void test_cb(Fl_Widget* w, void* v) {
  ...
  MyHBEdTable* p_table = (MyHBEdTable*)v;
...
MyHBEdTable::MyHBEdTable(int X, int Y, int W, int H, const char *l)
    ...
    mb.box(FL_NO_BOX);
//    mb.menu(pulldown);
    mb.add("В начало", 0, 0, 0);
    mb.add("В конец", 0, 0, 0);
    mb.add("Байт", 0, 0, 0);
    mb.add("Полуслово", 0, 0, 0);
    mb.add("Слово", 0, 0, 0);
        ...
        case _SH:
            {
                ...
                vec.at((unsigned int) n) = (unsigned short) (tmp & 0xff);
                vec.at((unsigned int) n+1) = (unsigned short) ((tmp>>8) & 0xff);
            }
            break;
        case _WD:
            {
                ...
                int tmp = stoul(s, nullptr, 16);
                // vec[row_edit*16 + col_edit] =  tmp & 0xff;
                auto n = row_edit * 16 + col_edit*4;
                vec.at((unsigned int) n) = (unsigned int) (tmp & 0xff);
                vec.at((unsigned int) n+1) = (unsigned int) ((tmp>>8) & 0xff);
                vec.at((unsigned int) n+2) = (unsigned int) ((tmp>>16) & 0xff);
                vec.at((unsigned int) n+3) = (unsigned int) ((tmp>>24) & 0xff);
            }
            break;
        }
        input->hide();
...
void MyHBEdTable::start_editing(int R, int C) {
    ...
    case _BT:
        ...
        break;
    case _SH:
        ss << std::hex << std::setfill('0') << std::setw(2) << (unsigned) vec[R * 16 + C*2+1]  << std::setw(2) << (unsigned) vec[R * 16 + C*2];
        break;
    case _WD:
        ss << std::hex << std::setfill('0')  << std::setw(2) << (unsigned) vec[R * 16 + C*4+3]  << std::setw(2) << (unsigned) vec[R * 16 + C*4 + 2]
